...
async function deleteCaptionController(req, res, next) {
  try {
    ...
    if (caption.createdBy.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "You are not authorized to delete this caption" });
    }
...
module.exports = {
  ...
  saveCaptionController,
  getCaptionsController,
};
